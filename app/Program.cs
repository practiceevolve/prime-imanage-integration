...
using PE.Mk2.Integrations.IManage.Filters;
...
builder.Services.AddSingleton<IManageServiceFactory>();
...
async Task RunOnStartup(ConfigService configService, Microsoft.Extensions.Logging.ILogger logger)
{
    ...
    var iManageServiceFactory = app.Services.GetService<IManageServiceFactory>();
    ...
    foreach (var tenantConfig in config.Tenants ?? [])
    {
        ...
        var iManageService = iManageServiceFactory.Create(tenantConfig.Prime.Tenant, tenantConfig.IManage);

        _ = queueInit(primeService);
        _ = queueInit(iManageService);
    }
